...
curl --request POST \
  ...
  --data '{
    "jql": "Sprint in openSprints() AND Sprint not in futureSprints() and status in (\"In Progress\") ORDER BY assignee ASC, issuetype ASC",
    "name": "AllInProgressTicketsInCurrentSprint",
    "description": "Lists all the in-progress tickets in the current sprint"
  }'
